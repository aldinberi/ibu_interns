var Utils = {
	user_login_check: function (page) {
		if (localStorage.getItem("userToken")) {
			...
			if ("PROFESSOR".localeCompare(user.type) == 0 && page != "professor.html") window.location = "professor.html";
			else if ("INTERN".localeCompare(user.type) == 0 && page != "intern.html") window.location = "intern.html";
			else if ("COMPANY_USER".localeCompare(user.type) == 0 && user.company_id != null && page != "company.html")
				window.location = "company.html";
			else if ("COMPANY_USER".localeCompare(user.type) == 0 && user.company_id == null && page != "index.html")
				window.location = "index.html";
			else if ("ADMIN".localeCompare(user.type) == 0 && page != "admin.html") window.location = "admin.html";
			else $("body").show();
		} else {
			if (page != "index.html") {
				window.location = "index.html";
	...
	download_file: function (file, file_name) {
		var doc = Utils.dataURItoBlob(file);
	...
	rescaleCaptcha: function (captcha) {
		//var width = $(captcha).parent().width();
		var width = $(".m-login__logo").width();
		console.log(width);
		var scale = 1;
		if (width < 302) {
			scale = width / 302;
	...
	sis_http_get: function (url, success_callback, error_callback) {
		$.ajax({
			url: url,
			type: "GET",
			beforeSend: function (xhr) {
				xhr.setRequestHeader("intern-user-token", localStorage.getItem("userToken"));
			},
